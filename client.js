...
User.prototype = {
	...
	,init: function() {
		js.Browser.window.alert("Client load init! id = " + this.id + ", name = " + this.name);
	}
	,__class__: User
}
...
hxdom.HtmlSerializer.prototype = $extend(haxe.Serializer.prototype,{
		...
		if(!Reflect.hasField(e.node,"__htmlSnippet")) {
			...
			while(_g < _g1.length) {
				...
				if(i.nodeType == 3) {
					...
					this.buf.b += Std.string(" " + Std.string(Reflect.field(text,"id")) + "-" + i.data.length);
				}
			}
		}
		this.buf.b += "'";
	...
	,closeTag: function(e) {
		this.buf.b += Std.string("</" + e.node.tagName.toLowerCase() + ">");
	}
	,openTag: function(e) {
		this.buf.b += Std.string("<" + e.node.tagName.toLowerCase());
		this.attrs(e);
	...
	,text: function(t) {
		this.buf.b += Std.string(t.node.data);
	}
